...
func GetAdmissionByTreatment(c *gin.Context) {
	var treatment_record entity.TreatmentRecord
	id := c.Param("id")
	if err := entity.DB().Raw("SELECT * FROM treatment_records WHERE admission_id = ? ", id).Find(&treatment_record).Error; err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
